...
const getApiKey = () => {
  ...
  if (!key) {
    console.error('API Key check - Constants.expoConfig?.extra?.apiKey:', Constants.expoConfig?.extra?.apiKey);
    console.error('API Key check - process.env?.EXPO_PUBLIC_API_KEY:', typeof process !== 'undefined' && 'env' in process ? process.env?.EXPO_PUBLIC_API_KEY : 'process not available');
    throw new Error(
...
Output: Return only a valid JSON array of exactly 3 strings, no additional text.`,
    ...
    if (!jsonText.trim()) {
      console.error('API Response - Empty text:', JSON.stringify(firstCandidate.content.parts, null, 2));
...
const generateOrEditImage = async (base64Image: string, prompt: string): Promise<string> => {
  try {
    ...
    console.error('API Response - No image data:', JSON.stringify(firstCandidate.content.parts, null, 2));
...
export const findTattooArtists = async (tattooStyle: string): Promise<TattooArtist[]> => {
  ...
  try {
    ...
    if (chunks.length === 0) {
      console.warn('No Maps Grounding chunks found. Response:', JSON.stringify(response.candidates?.[0], null, 2));
